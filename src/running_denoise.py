...
def run_denoise_entropy(de):
    if de.part == 1:
        ...
        ##### a partir d'aqui el foor loop per cada valor de allowed_lengths
        ...
        for len_seq in allowed_lengths:
                    ...
                    for mother in tqdm(mothers_ratio):
                        row = [
                            ...
                            list(desub.data_initial.loc[
                                     list(pd.Series(desub.denoised_ratio_output) == mother), desub.abund_col_names].sum(0)) +
            ...
            if (desub.output_type == 'd') or (desub.output_type == 'all'):
                ...
                if desub.cores > 1:
                    ...
                    for mother in tqdm(mothers_d):
                        row = [
                            desub.good_mothers[list(desub.good_mothers.loc[:, 'id'] == mother)][de.first_col_names].values.tolist()[
                                0] +
                            list(desub.data_initial.loc[list(pd.Series(desub.denoised_d_output) == mother), desub.abund_col_names].sum(
                                0)) +
                            desub.good_mothers[list(desub.good_mothers.loc[:, 'id'] == mother)][de.seq].values.tolist()]
                        row = pd.Series(row[0], index=[de.first_col_names + desub.abund_col_names + [de.seq]][0])
            ...
            if (desub.output_type == 'ratio_d') or (desub.output_type == 'all'):
                desub.good_mothers = desub.data_initial.loc[desub.good_seq][de.first_col_names + desub.abund_col_names + [de.seq]]
                # writing ratio_d
                if desub.cores > 1:
                    pool = mp.Pool(desub.cores)
                    ...
                    pool.close()
                    del pool
                    desub.denoised_ratio_d = pd.DataFrame(row, columns=[de.first_col_names + desub.abund_col_names + [de.seq]][0])
                    ...
                    desub.denoised_ratio_d = pd.DataFrame(columns=[de.first_col_names + desub.abund_col_names + [de.seq]][0])
                    for mother in tqdm(mothers_ratio_d):
                        row = [
                            desub.good_mothers[list(desub.good_mothers.loc[:, 'id'] == mother)][de.first_col_names].values.tolist()[
                                0] +
